...
function ParsingBadge({ status, errors }: { status: string; errors?: string[] }) {
  if (status === 'success') return <span className="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-full" style={{ backgroundColor: '#E8F5EE', color: '#1B6B40' }}>OK</span>;
  if (status === 'partial') {
...
function StatusBadge({ statut }: { statut: string }) {
  if (statut === 'OK') return <span className="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-full" style={{ backgroundColor: '#E8F5EE', color: '#1B6B40' }}>OK</span>;
  ...
  return <span className="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-full bg-red-100 text-red-800">Retard</span>;
}
...
export function PageDonnees() {
  ...
  const moisCouverts = new Map<string, string>();
      ...
      <div className="flex gap-1 mb-5 border-b border-slate-200">
        {(['decades', 'mois', 'regles'] as Tab[]).map(t => (
          <button
            ...
            style={tab === t
              ? { borderBottomColor: '#1B6B40', color: '#1B6B40' }
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button
              ...
              disabled={uploading}
              className="flex items-center gap-2 px-4 py-2 text-white text-sm font-semibold rounded disabled:bg-slate-300 transition-colors"
            ...
            <button
              onClick={handleScan}
              disabled={scanning}
              className="px-4 py-2 text-sm font-medium border border-slate-200 rounded hover:bg-slate-50 disabled:bg-slate-100 transition-colors"
            >
            ...
            <span className="text-xs text-slate-400 data-val">{relevesFiltres.length} décade(s)</span>
            {releves.length > 0 && (
              <button
                onClick={() => setShowConfirmClear(true)}
                className="ml-auto px-3 py-2 text-xs font-medium text-red-600 border border-red-200 rounded hover:bg-red-50 transition-colors"
              >
                Tout supprimer
              </button>
            )}
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  {relevesFiltres.map(r => (
                    <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3 text-slate-700 font-medium data-val">{r.annee}</td>
                      <td className="px-4 py-3 text-slate-600">{formatMoisLabel(`${r.annee}-${String(r.mois).padStart(2, '0')}`)}</td>
                      <td className="px-4 py-3">
                        <span className="inline-flex w-6 h-6 items-center justify-center text-[10px] font-bold rounded-full"
                          style={{ backgroundColor: '#E8F5EE', color: '#1B6B40' }}>D{r.decade}</span>
                      </td>
                      <td className="px-4 py-3 text-right font-medium text-slate-800 data-val">{formatEuros(r.totalNetHT)}</td>
                      <td className="px-4 py-3 text-right text-red-600 data-val">
      ...
      {tab === 'mois' && (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-slate-400 data-val">{analyses.length} mois chargés</span>
          </div>
                ...
                {analyses.map(a => {
                  ...
                  return (
                    <tr key={a.mois} className="hover:bg-slate-50 transition-colors">
                        ...
                        <div className="flex gap-1 justify-center">
                          {[1, 2, 3].map(d => (
                            <span
                              ...
                              style={a.decadesPresentes.includes(d)
                                ? { backgroundColor: '#E8F5EE', color: '#1B6B40' }
                      ...
                      <td className="px-4 py-3 text-right text-slate-600 data-val">{formatEuros(a.remiseAttendue)}</td>
                      <td className="px-4 py-3 text-right text-slate-600 data-val">{formatEuros(a.remiseReelle)}</td>
                      <td className="px-4 py-3 text-right text-slate-500 data-val">{a.fraisGeneraux > 0 ? formatEuros(a.fraisGeneraux) : '—'}</td>
                      <td className="px-4 py-3 text-right text-slate-600 data-val">{formatEuros(a.reversee)}</td>
                      <td className={`px-4 py-3 text-right font-semibold data-val ${a.delta < -0.01 ? 'text-red-600' : 'text-emerald-600'}`}>
                        {formatEuros(a.delta)}
                      </td>
                      <td className="px-4 py-3"><StatusBadge statut={a.statut} /></td>
                      <td className="px-4 py-3">
                        {claimRef
                          ? <span className="text-[10px] px-2 py-0.5 rounded font-medium" style={{ color: '#1B6B40', backgroundColor: '#E8F5EE' }}>{claimRef}</span>
                          : a.statut === 'RETARD'
        ...
        <div className="space-y-5 max-w-3xl">
          ...
          <div className="bg-white border border-slate-200 overflow-hidden" style={{ borderRadius: '4px' }}>
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div>
                ...
                <div className="bg-slate-50 px-4 py-3 font-mono text-xs text-slate-700 space-y-0.5" style={{ borderRadius: '4px' }}>
              ...
              <div>
                ...
                <div className="bg-slate-50 px-4 py-3 font-mono text-xs text-slate-700" style={{ borderRadius: '4px' }}>
                  <p className="font-semibold" style={{ color: '#1B6B40' }}>Attendue(M)  =  Assiette(M) × 3 %</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-slate-50 px-4 py-3 font-mono text-xs text-slate-700" style={{ borderRadius: '4px' }}>
                  <p className="font-semibold" style={{ color: '#1B6B40' }}>Delta(M)  =  Reversée(M) − Attendue(M)</p>
                </div>
          ...
          <div className="bg-white border border-slate-200 overflow-hidden" style={{ borderRadius: '4px' }}>
            ...
            <div className="divide-y divide-slate-100">
              {[
                {
                  badge: <span className="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-full" style={{ backgroundColor: '#E8F5EE', color: '#1B6B40' }}>OK</span>,
                ...
                {
                  badge: <span className="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-full bg-red-100 text-red-800">Retard</span>,
